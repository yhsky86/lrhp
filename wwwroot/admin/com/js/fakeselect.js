...
// 안드로이드 브라우저의 select focus() 이벤트 오동작을 위한 추가 함수 패치 //fakeselect.initialize(); 시작 함수 호출
	...
	function rc(sel,v,f){
		...
		if(v.widthminus==undefined){
			...
			for(var i=0; i<4; i++) {
				var widthInt = 0;
				try {
					widthInt = parseInt(getstyle(t,cklist[i]));
					if (widthInt == null || widthInt == undefined || isNaN(widthInt)) {
						widthInt = 0;
					}
				} catch(e) {}
				v.widthminus+=widthInt;
			}
		...
		if(!sel.multiple){//normal
			...
			ac.setoptions=function(){
				...
				try {
					...
					sel.parentNode.insertBefore(ac,((sel.parentNode.hasChildNodes())?sel.parentNode.childNodes[0]:null));
				}
...
if(window.addEventListener){
	...
	document.write('<script id="deferscript" defer="defer" src="/js/front/travel/empty.js"></script>');
